  ...
  sections: [{
    ...
    children: [
      ...
      BOX([{ t: "従来の「1件あたり何分」と、リスト構成との関係", size: 20, color: "404040" },
           { t: "相関 r ＝ 0.45　（p ＝ 0.015、29名）", size: 24, bold: true, color: "C00000" },
      ...
      table([3000, 3000, 3000], [
        ["段取りの設定", "1.6分に必要な1社あたり件数", "実績 2.34 件との関係"],
        ["見直し前 92.1 秒/社", "4.51 件", "1.93 倍が必要 — 事実上不可能"],
      ...
      table([2600, 2100, 4300], [
        ["工程", "目標達成率", "内容"],
        ["梱包", "100.7 ％", "実測 1.589 分/件。目標に届いています。"],
      ...
      BOX([{ t: "ピッキングの未達 19.2 秒/件 の内訳", size: 20, color: "404040" },
           { t: "速さ由来 15.6 秒（81％）　／　リスト構成由来 3.6 秒（19％）", size: 23, bold: true },
           { t: "8割は日々の作業で埋まる分。2割はリストの組み方で決まる分です。", size: 20, color: "404040" }], "FFF8E7"),
      P("速さ由来の 15.6 秒は、実測時間と標準時間の差です。ここは作業のしかたを見直せば縮みます。リスト構成由来の 3.6 秒は、1社あたりが 2.34 件しかないために生じる分で、担当者の努力では動かせません。1.6分に収めるには 1社あたり 2.75 件が必要で、この 18％ はリストの組み方の課題です。", { before: 120 }),
      P("この内訳は印刷ページの下部に毎月表示されます。数字が変われば自動で計算し直されます。"),
      P("一人ひとりの判定も、この達成率で行います。ピッキング・梱包の各シートに、次の4段階で表示されます。", { before: 120 }),
      table([2200, 3200, 3600], [
        ["判定", "達成率", "意味"],
        ["◎ 達成", "100％以上", "会社目標に届いています"],
        ["○ あと一歩", "90〜100％", "あと1割以内です"],
        ["△ 要改善", "80〜90％", "作業のしかたを見直しましょう"],
        ["✕ 要支援", "80％未満", "支援を優先して届けます"],
      ]),
      P("しきい値（90・80）は「設定」シートで変更できます。◎の基準100％は会社目標そのものです。", { before: 120 }),
      P("各シートの「順位」列も、この同じ達成率で付けています。判定と順位が別々の物差しになることはありません。順位が上なのに判定が低い、といった食い違いは起きません。", { before: 120 }),
      ...
      table([2600, 6400], [
        ...
        ["少ない記録日数で決めつけられない",
         "記録日数が少ないと数字はブレます。各シートには95％の幅（下限・上限）を参考表示しており、幅が広い人は記録日数が足りない合図です。幅が広い人の1か月の判定は、そのまま鵜呑みにせず数か月をならして見ます。"],
        ["どこを直せばよいかが分かる",
      ...
      BULLET("段取りの見直しで、全員の数字がいったん下がりました。全体の平均（件数で重みづけ）は、見直し前の係数で計算すると 103.4％、見直し後は 89.7％ です。これは皆さんの働きが落ちたのではなく、基準を厳しい側に直したためです。", true),
      ...
      BULLET("段取りを除いたあとも、リスト構成との相関は完全には消えません（相関 r ＝ 0.25、p ＝ 0.18）。統計的には「傾きは無いと言ってよい」範囲ですが、補正しきれていない分が残っている可能性があります。"),
      BULLET("ピッキングの順位は月ごとにブレます。ブレの大きさを示す再現性の数値は、段取り見直し後のデータで計算し直すと 0.40 でした（1に近いほど安定）。1か月の順位だけで人を判断せず、数か月をならして見る前提で運用します。梱包は 0.87 で比較的安定しています。"),
      BULLET("時間の記録漏れがあると、その人の効率は実力以上に高く出ます。記録の正確さが、このしくみの前提です。"),

      H1("8. 誰が何を見るか"),
      P("全員の氏名と順位が並んだ一覧は、係長が見ます。改善の支援をどこから始めるかを決めるための資料です。"),
      P("並びが2種類あるので、見誤らないようにご注意ください。", { before: 120 }),
      table([3200, 5800], [
        ["どこの並びか", "並び方"],
        ["ピッキング・梱包シートの「順位」列", "達成率の高い順。1位は最も目標に近い人です。"],
        ["印刷ページの一覧", "達成率の低い順。上にいる人ほど、支援を先に届ける相手です。"],
      ]),
      BOX([{ t: "印刷ページの一覧で上にいることは、「最も成績が悪い人」という意味ではありません。", size: 22, bold: true },
      ...
      BULLET("95％の幅が重なる範囲の順位差を、実力差として扱わないこと"),
